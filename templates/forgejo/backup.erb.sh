...
if [[ ${status} -ne 0 ]]; then
  ...
  PASSPHRASE="<% @platform_sunet_se_gpg_password %>" duplicity --encrypt-key=<%= @smtp_user %>@sunet.se --full-if-older-than 1M --asynchronous-upload "${backup_dir}" rclone://backups:/backups
  second_status=${?}
