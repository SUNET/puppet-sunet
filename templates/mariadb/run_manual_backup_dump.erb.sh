...
docker exec mariadb-db-1 mysql -u root -p'<%= @mysql_root_password %>' -e "GRANT REPLICATION SLAVE ON *.* TO 'backup'@'%'"
echo "Running backup as root user"
docker exec mariadb-db-1 mysqldump "${dump_args}" | gzip >"/backups/${dump_name}"
