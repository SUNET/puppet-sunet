class sunet::gitolite(
    ...
    $_ssh_key = $ssh_key ? {
        undef   => lookup('gitolite-admin-ssh-key', undef),
        ''      => lookup('gitolite-admin-ssh-key', undef),
        default => $ssh_key
    }
    ...
    if $save_private_admin_key_on_server {
        ...
        $gitolite_initial_public_admin_key = lookup('gitolite-initial-public-admin-key', undef)
        file { "${home}/admin.pub":
            ensure  => file,
            owner   => 'root',
            group   => 'root',
