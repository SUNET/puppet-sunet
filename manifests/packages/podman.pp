...
class sunet::packages::podman(
  ...
  exec { 'podman_repo_key':
    ...
    command => "curl https://download.opensuse.org/repositories/devel:/kubic:/libcontainers:/stable/${reponame}_${::operatingsystemrelease}/Release.key | gpg --dearmor > /etc/apt/trusted.gpg.d/libcontainers.gpg",
