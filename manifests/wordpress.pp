  ...
  if (!$db_host) {
    ...
    if ($mysql_dump) {
      ...
      -> cron { "${name}_mysql_dump":
        command => "mysqldump -h $(/usr/local/bin/get_container_ip ${name}_mysql) -u ${db_user} -p${pwd} ${db_name} " +
                    "> /data/${name}/dump/${db_name}.sql.new && test -s /data/${name}/dump/${db_name}.sql.new " +
                    "&& mv /data/${name}/dump/${db_name}.sql.new /data/${name}/dump/${db_name}.sql",
        user    => 'root',
        minute  => '*/5'
      }
    }
